...
func TestEncodeDecodeStringBytes(t *testing.T) {
	...
	for _, c := range tests {
		n, err := zcoder.En.EncodeString(wr, c.value)
		assert.Nil(t, err)
		assert.Equal(t, c.length, n)
		v, n, err := zcoder.De.DecodeString(wr)
		assert.Nil(t, err)
		assert.Equal(t, c.length, n)
		assert.Equal(t, c.value, v)

		n, err = zcoder.En.EncodeBytes(wr, []byte(c.value))
		assert.Nil(t, err)
		assert.Equal(t, c.length, n)
		b, n, err := zcoder.De.DecodeBytes(wr)
		assert.Nil(t, err)
		assert.Equal(t, c.length, n)
	...
	for i := 0; i < 100; i++ {
		s := randString()
		n, err := zcoder.En.EncodeString(wr, s)
		assert.Nil(t, err)
		v, n2, err := zcoder.De.DecodeString(wr)
		assert.Nil(t, err)
		assert.Equal(t, n, n2)
		assert.Equal(t, s, v)

		n, err = zcoder.En.EncodeBytes(wr, []byte(s))
		assert.Nil(t, err)
		b, n2, err := zcoder.De.DecodeBytes(wr)
		assert.Nil(t, err)
		assert.Equal(t, n, n2)
...
func TestEncodeDecodeFloat(t *testing.T) {
	...
	for _, v := range []float32{
		...
		n, err := zcoder.En.EncodeFloat32(wr, v)
		assert.Nil(t, err)
		v2, n2, err2 := zcoder.De.DecodeFloat32(wr)
		assert.Nil(t, err2)
		assert.Equal(t, n, n2)
		assert.Equal(t, v, v2)
	}
	for _, v := range []float64{
		...
		n, err := zcoder.En.EncodeFloat64(wr, v)
		assert.Nil(t, err)
		v2, n2, err2 := zcoder.De.DecodeFloat64(wr)
		assert.Nil(t, err2)
		assert.Equal(t, n, n2)
		assert.Equal(t, v, v2)
	}
